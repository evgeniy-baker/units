...
function f4() {
    let push = document.querySelector('.i-4').value;
    d1.push(push);
    showArr('.out-4', d1);
}
...
function f5() {
    let unshift = document.querySelector('.i-5').value;
    d1.unshift(unshift);
    showArr('.out-5', d1);
}
...
function f6() {
    let unshift = document.querySelector('.i-6').value;
    d6[d6.length] = unshift
    showArr('.out-6', d6);
}
...
// а потом заполняет массив d14 так, что количество элементов равно введенному числу, 
// и каждый элемент равен 1. Т.е. пользователь ввел 5, массив будет [1,1,1,1,1]. 
// Выведите массив в out-14.
// Во всех задачах где говорим второй, третий и т.д. имеем ввиду индекс
...
function f15() {
    let index = +document.querySelector('.i-15').value;
    if (d15.indexOf(index) === -1) {
        d15.push(index);
    }
    showArr('.out-15', d15);
}

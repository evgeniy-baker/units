...
const f1 = () => {
    ...
    let item = +document.querySelector('.i-1').value
    document.querySelector('.out-1').innerHTML = a1.includes(item)
}
...
// Нахождение индекса либо через indexOf или через цикл. Сравните данную задачу с задачей 2 из юнита 1. Подумайте какой из методов: includes, indexOf
// более удобен для данной задачи. Как тестируется - ввожу числа и проверяю результат.


let a2 = [4, 12, 4, 2, 15, 98];

const f2 = () => {
    let item = +document.querySelector('.i-2').value
    let out = document.querySelector('.out-2')
    if (a2.includes(item) === true) return out.innerHTML = a2.indexOf(item)
    else return out.innerHTML = false
}
...
const f3 = () => {
    let item = +document.querySelector('.i-3').value
    let out = document.querySelector('.out-3')
    for (let key in a3) {
        for (i of a3[key]) {
            if (item === i) return out.innerHTML = true
...
const f8 = () => {
    let str = document.querySelector('.i-8').value
    let out = document.querySelector('.out-8')

    out.innerHTML = a8.includes(str)
}

...
// Task 4. Создайте на странице input[type=number] с классом i-4, куда пользователь может ввести год своего
...
function f4(){
    let date = +document.querySelector('.i-4').value
    let age = document.querySelector('.out-4')
    if (2023 - date >= 18){
        age.innerHTML = 1
    }
    else {
...
// Есть кнопка b-5 которая запускает функцию f5. Функция должна вывести в  .out-5 символ m если число
// меньше нуля, 0 если число равно нулю и 1 если больше.
...
// Есть кнопка b-6 которая запускает функцию f6. Функция должна вывести в  .out-6  слово even если число
// четное и odd если нечетное. Для проверки четности используется целочисленный остаток от деления на
// 2 (оператор %). Если остаток равен нулю  - четное, нет - нечетное.
...
// При нажатии кнопки b-7 срабатывает функция f7. Функция должна число из i-71 возвести в степень i-72,
...
// Функция должна получить выбранное в select число, потом с помощью
// switch case сравнить его поочередно с ‘1’, ‘2’, ‘3’. И если число выбрано - 1,
// то вывести в out-8 строку one, если 2 - two, если 3 - three.

function f8(){
    let select = document.querySelector('.s-8').innerHTML
...
function f9(){ 
    ...
    switch (true){
        case (select >= 1 && select <= 32):
            out.innerHTML = 1;
            break
        case (select >= 33 && select <= 43):
            out.innerHTML = 2;
            break
        case (select >= 44 && select <= 64):
            out.innerHTML = 3;
            break
        default:
            out.innerHTML = 0;

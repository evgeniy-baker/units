...
// TASK 02
// Выполним эмуляцию метода isArray. В качестве критерия будем проверять следующее - содержимое переменной можно перебрать с помощью цикла for let i = .
// Т.е. все индексы должны быть числами. Функция должна выводить в out-2 true или false, в зависимости от того, что лежит в a2.
...
// Обратите внимание на начальное значение accum и значения, учитывая что мы не делаем return. Обратите внимание,
...
// Результат - сумму положительных элементов массива выводится в out-8. Обратите внимание, что если вы будете решать предыдущим методом, что получите странный результат.
...
const f8 = () => {
    let res = a8.reduce((accum, item) => {
        ...
        return accum
    }, 0)   // Потому, что accum будет -2, поэтому accum делаем 0
    console.log(res)
}
...
// TASK 15
// По нажатию b-15 выполняется функция f15. Функция должна перебрать массив a15 измерений температуры за сутки  и вывести в out-15 среднюю арифметическую температуру.
// Применяем reduce.
...
const f15 = () => {
    ...
    document.querySelector('.out-15').innerHTML = res / d;
}

// TASK 16
// По нажатию b-16 выполняется функция f16. Функция должна выполнить переиндексацию массива a16
// и получить объект который состоит из ключей - id и значений name. Применяем reduce.
...
const f16 = () => {
    let res = a16.reduce((accum, item) => {
        return accum += `${item.id} : ${item.name} <br>`
    }, '')
...
// TASK 18
// По нажатию b-18 выполняется функция f18. Функция должна вывести в out-18 массив a18 в формате
// ключ пробел значение перенос строки. Для формирования строки для вывода используйте reduce.
...
// TASK 19
// По нажатию b-19 выполняется функция f19. Функция должна вывести в out-19 массив a19 в формате
// ключ пробел значение перенос строки. Для формирования строки для вывода используйте reduce.
